...
const NewChatRight = ({ folder }: { folder?: string }) => {
  ...
  return (
    <a
      className={`newchat flex flex-1 items-center rounded-md hover:bg-gray-500/10 transition-all duration-200 text-black text-sm flex-shrink-0 ${
        generating
    ...
    >
      {folder ? (
        ...
        <>
          <PlusIcon />
          <span className='inline-flex text-black text-sm'>{t('newChat')}</span>
        </>
      )}
    </a>
  );
